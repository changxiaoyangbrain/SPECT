...
- 迭代次数 (Iterations): 10。实验表明 10 次迭代后似然函数趋于平稳。""",
                    ...
                    },
                    {
                        ...
                        "image": os.path.join(os.path.dirname(os.path.abspath(__file__)), "pictures", "viz_compare_raw_axial.png"),
...
本实验中，原始重建结果的 RMSE 为 0.209，说明整体准确度尚可。为了抑制噪声，我们采用了后处理滤波（Post-filtering）。对比图 2 显示，经 FWHM=10mm 高斯滤波后，图像平滑度显著提升，RMSE 降至 0.128。""",
                        "image": os.path.join(os.path.dirname(os.path.abspath(__file__)), "pictures", "viz_compare_filtered_axial.png"),
